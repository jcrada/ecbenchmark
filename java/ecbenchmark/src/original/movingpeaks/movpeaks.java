...
import ecbenchmark.movingpeaks.PeakFunctionCone;
import ecbenchmark.movingpeaks.PeakFunctionOne;
import ecbenchmark.movingpeaks.Scenario;
import java.util.*;
...
public class movpeaks {
    ...
    static final int PEAKFUNCTION1 = 0;
    static final int PEAKFUNCTIONCONE = 1;
    static final int PEAKFUNCTIONSPHERE = 2;
    ...
    public movpeaks(Scenario scenario) {
        this(scenario.getNumberOfPeaks(),
                scenario.getNumberOfDimensions(),
                scenario.getChangeFrequency(),
                scenario.getVlength(),
                scenario.getMinWidth(),
                scenario.getMaxWidth(),
                scenario.getStandardWidth(),
                scenario.getDirectionMovement(),
                scenario.getHeightSeverity(),
                scenario.getWidthSeverity(),
                (scenario.getPeakFunction() instanceof PeakFunctionOne
                ? PEAKFUNCTION1
                : (scenario.getPeakFunction() instanceof PeakFunctionCone
                ? PEAKFUNCTIONCONE
                : PEAKFUNCTIONSPHERE)),
                scenario.getSeed());
        minheight = scenario.getMinHeight();
        maxheight = scenario.getMaxHeight();
        standardheight = scenario.getStandardHeight();
        
        mincoordinate = scenario.getMinCoordinate();
        maxcoordinate = scenario.getMaxCoordinate();
        use_basis_function = (scenario.getBasisFunction() != null);
    ...
    public movpeaks(int numberOfAttractors,
        ...
        System.out.println("PeakFunction:" + pf.getClass().getName());
    ...
    public void change_peaks() {
        int i, j;
        double sum= 0, sum2 = 0, offset= 0, dummy = 0;
        ...
        for (i = 0; i < this.number_of_peaks; i++) {
            ...
            for (j = 0; j < geno_size; j++) {
                ...
                } else if ((peak[i][j] + this.prev_movement[i][j])
                        > this.maxcoordinate) {
                    peak[i][j] =
    ...
    public double get_current_error() /* returns the error of the best individual evaluated since last change */ /* To use this function, calculate_average_error and calculate_offline_performance must be set */ {
